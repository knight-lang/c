...
char*j,*k,*BUF[]={"false","true","null"},*MAPI[MAXVARS],NB[22];
...
l p(){
	...
	*u=c;
	...
	I(SPN(u,"TFN"))R c-84?c-70?12:0:8;
...
l r(l v) {
	...
	C('!')8*!b(A1)
	C('L')1|L(s(A1))<<4
	C('D')(1&(t=r(A1))?P("Number(%lld)",t>>4):2&t?P("String(%s)",S(t)):
			12-t?t<13?P("Boolean(%s)",t?"true":"false"):P("<?>"):P("Null()")),
			P("\n"),t
	...
	C('>')
		(i=1&(t=r(A1))?(t>>4)-n(A2):
			2&t?CMP(S(t),s(A2)):
			(t/8)-b(A2)),8*(G(0)=='<'?i<0:i>0)

	C('?')8*((t=r(A1))==(t2=r(A2))||t&t2&2&&!CMP(S(t),S(t2)))
	C('&')b(t=r(A1))?r(A2):t
	C('|')b(t=r(A1))?t:r(A2)
	C(';')r(A1),r(A2)
	D'=':
	...
	D'W':W(b(A1))r(A2);R 12
	C('I')r(G(3-b(A1)))
	C('G')2|(l)NDUP(s(A1)+n(A2),n(G(3)))
	C('S')
		j=s(A1),
		t=n(A2),
		t2=n(G(3)),
