...
DECLARE_FUNCTION(call, 1, "CALL") {
	...
	kn_ast_clone(ast); // We subtracted one, so now we have to add.
...
DECLARE_FUNCTION(then, 2, ";") {
	...
	kn_value lhs = kn_ast_run(kn_value_as_ast(args[0]));
...
DECLARE_FUNCTION(while, 2, "WHILE") {
	while (kn_value_to_boolean(args[0])) {
		kn_value result = kn_value_run(args[1]);

#ifdef KN_USE_REFCOUNT
		kn_value_free(result);
#else
		(void) result;
#endif /* KN_USE_REFCOUNT */
