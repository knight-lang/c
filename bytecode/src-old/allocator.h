...
# define KN_HEADER alignas(KN_VALUE_ALIGNMENT) size_t refcount; \
                   unsigned char flags;
# define KN_RC_FREE(kind, expr) do { kn_##kind##_free(expr); } while(0)
# define KN_RC_CLONE(kind, expr) (kn_##kind##_clone(expr))
#else
# define KN_HEADER alignas(KN_VALUE_ALIGNMENT) unsigned char flags;
# define KN_RC_FREE(kind, expr) do { (void) expr; } while(0)
# define KN_RC_CLONE(kind, expr) (expr)
